...
pub fn main() !void 
{
    ...
    for (as_input.items) |*i| {
        //std.debug.print(">>{s}\n", .{i.*});
        const i32_clicks: i32 = try std.fmt.parseInt(i32, i.*[1..], 10);
        switch (i.*[0]) {
            'L' => {
                i32_dial_points_at = @mod(i32_dial_points_at - i32_clicks, 100);
                //std.debug.print("left  {d} -> {d}\n", .{i32_clicks, i32_dial_points_at});
            },
            'R' => {
                i32_dial_points_at = @mod(i32_dial_points_at + i32_clicks, 100);
                //std.debug.print("right {d} -> {d}\n", .{i32_clicks, i32_dial_points_at});
            },
            else => unreachable,
        }
